...
- Fichiers principaux :
  ...
  - `assets/js/app.js` : ancien script redondant/non utilisé (non référencé dans `index.html`).
...
- [ ] **Listes de sections dans `index.html` (`.paragraph-card`)**
...
- [ ] **Créer un module JS de données** (approche privilégiée dans un premier temps)
...
- [ ] **Centraliser la définition des sections**
...
- [ ] **Gérer `assets/js/app.js` (script legacy)**
...
- [ ] **Découper `main_core.js` en plusieurs modules** (éventuellement dans un second temps)
...
1. [ ] **Extraction des données de prompts (`PROMPT_TYPES` + `TEMPLATES`) dans un module dédié**

   - Gros impact sur la taille et la lisibilité de `main_core.js`.
   - Centralisation des données métier.

2. [ ] **Nettoyage de `app.js` (suppression/déplacement)**

   - Réduit le bruit dans `assets/js`.

3. [ ] **Génération dynamique des `.paragraph-card`**

   - Allège `index.html` et centralise la définition des sections.

4. [ ] **Découpage de `main_core.js` en sous-modules (UI, stockage, analyse)**
